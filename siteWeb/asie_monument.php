...
<html lang="fr">
	...
	<h1>Niveau 1</h1>
	<h2>Asie - Monument</h2>
	<form action="asie_monument.php" method="POST">
	<div>
		<br>
		<label>Je suis une structure architecturale importante pour la guerre en Asie.</label>
	...
	<div>
		<button id="indice" name="indice1">Indice 1</button>
		<button id="indice" name="indice2">Indice 2</button>
		</div>
	</div>
				...
				if(isset($_SESSION['email']))
					{
						...
						if($connexion > 0)
						{
							...
							if(isset($_POST['submit']))
							{
								...
								if($reponse==$reponse_1_mon)
								{									
									$request="UPDATE user SET 1_mon='1' WHERE email='$email'";
									$result=mysqli_query($connexion,$request);
									echo "<script>valid();</script>";
									echo "<br><a href='asie_sport.php'>Enigme suivante</a>";									
